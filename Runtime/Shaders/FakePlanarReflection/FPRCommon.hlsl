...
struct Attributes
{
	...
	float4 uv1 : TEXCOORD1; // rgb = GI, a = baked cycle rate
	float4 uv2 : TEXCOORD2; // xyz = baked growth offsets, w is growth progression
	float2 uv3 : TEXCOORD3; // xy = baked local xz pivots (for objects with several pivot like billboard)
};
...
float _TimeSpeed, _CycleDuration;
float _ProgressionRange;
float _Luminosity, _Saturation;
float _ReflectionDeformFreq, _ReflectionDeformAmp;
float4 _ReflectionColor;
float _ReflectionTimeFactor;
float _PlaneHeight;
...
float CycleValue(float2 seed)
{
	float time = _Time.y * (1.0 / max(1e-06, _CycleDuration)) * _TimeSpeed;
	return RandomRange01(seed) + time;
}
...
Varyings Vert(Attributes input, bool isReflection)
{
	...
	// Find pivot position : just add local offsets xz sotred in uv3 to the object position 
	float3 pivotPosition = GetObjectPositionWS() + float3(input.uv3, 0);
	
	// Random range based on horizontal position
	float2 cycleSeed = _PerPivotLifeCycle > 0.5 ? pivotPosition.xz : GetObjectPositionWS().xz;
	float cycleValue = frac(CycleValue(cycleSeed));
	
	// Setup growth and decay levels
	float growthValue = saturate(cycleValue * 2.0); // growth happens half of the cycle
	float decayValue = saturate((cycleValue * 2.0) - 1.0); // decay happens the other half of the cycle
	...
	// Water plane offset
	float2 waterPlaneUV = 0.5 - (pivotPosition.xz / _PlaneSize);
	...
	float decayAlphaMask = decayValue * 2 - 1; // the fade happen in the second half of decay
...
{
	// Debug mode: return solid debug color
	if (_DebugMode > 0.5)
	{
		return _DebugColor;
	}
	...
	float decayColorMask = EaseOutCirc(input.values.x);
	...
	if (abs(viewDir.y) > eps)
	{
		...
		else
		{
			if (t > 0.0)
			{
				...
				planeUV = planePositionWS.xz;
			}
		}
		
	}
	
	half UVDeformFactor = SAMPLE_TEXTURE2D(_ReflectionNoiseMap, sampler_ReflectionNoiseMap, planeUV * 0.5).r;
	float2 UVDeform = planeUV + UVDeformFactor * 0.5 + _Time.y * _ReflectionTimeFactor * _TimeSpeed;
